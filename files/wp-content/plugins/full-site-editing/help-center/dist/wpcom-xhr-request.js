...
var Agent = __webpack_require__(66545);
...
/* harmony import */ var superagent__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(139);
...
/* harmony import */ var wp_error__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(82884);
 ...
 * Module variables
 */
const debug = debug__WEBPACK_IMPORTED_MODULE_0___default()('wpcom-xhr-request');
  ...
  req.end((error, response) => {
    ...
    if (!req.xhr) {
      ...
      if (processResponseInStreamMode) {
        return fn(new Error('stream mode processing is not yet implemented for Node.js'), body, headers);
      }
    }
    const {
      ok
    ...
    if (ok) {
      ...
      if (isEnvelopeMode && processResponseInEnvelopeMode || processResponseInStreamMode) {
        ...
        if (null !== statusCode && 2 !== Math.floor(statusCode / 100)) {
          ...
          const wpe = wp_error__WEBPACK_IMPORTED_MODULE_2___default()({
            path,
            method
          }, statusCode, body);
...
function request(options, fn) {
  ...
  const req = (superagent__WEBPACK_IMPORTED_MODULE_1___default())[method](settings.url);
