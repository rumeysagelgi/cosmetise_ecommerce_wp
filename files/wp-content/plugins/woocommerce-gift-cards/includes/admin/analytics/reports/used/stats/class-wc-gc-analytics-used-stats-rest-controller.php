...
/**
 * WC_GC_Analytics_Used_Stats_REST_Controller class.
 *
 * @version 1.16.0
 */
class WC_GC_Analytics_Used_Stats_REST_Controller extends WC_REST_Reports_Controller {
	 ...
	 * @var string
	 */
	protected $rest_base = 'reports/giftcards/used/stats';
	...
	public function get_items( $request ) {
		...
		$query = new WC_GC_Analytics_Used_Stats_Query( $query_args );
		try {
	...
	public function prepare_item_for_response( $report, $request ) {
		 ...
		 */
		return apply_filters( 'woocommerce_rest_prepare_report_giftcards_used_stats', $response, $report, $request );
	}
	...
	public function get_item_schema() {
		$data_values = array(
			'giftcards_count'   => array(
				...
				'description' => __( 'Number of items sold.', 'woocommerce-gift-cards' ),
				'type'        => 'integer',
		...
		$schema = array(
			...
			'title'      => 'report_giftcards_used_stats',
			'type'       => 'object',
			'properties' => array(
				'totals'    => array(
	...
	public function set_default_report_data( $results ) {
		if ( empty( $results ) ) {
			...
			$results->totals->used_amount        = 0;
			$results->totals->refunded_amount    = 0;
			$results->totals->net_amount         = 0;
