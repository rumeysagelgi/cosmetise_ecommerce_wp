...
class WC_GC_Analytics_Expired_Stats_REST_Controller extends WC_REST_Reports_Controller {
	 ...
	 * @var string
	 */
	protected $rest_base = 'reports/giftcards/expired/stats';

	/**
	 * Mapping between external parameter name and name expired in query class.
	 *
	 * @var array
	 */
	...
	public function get_items( $request ) {
		...
		$query = new WC_GC_Analytics_Expired_Stats_Query( $query_args );
		try {
	...
	public function prepare_item_for_response( $report, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request expired to generate the response.
		 */
		return apply_filters( 'woocommerce_rest_prepare_report_giftcards_expired_stats', $response, $report, $request );
	}
	...
	public function get_item_schema() {
		$data_values = array(
			'giftcards_count'   => array(
				'title'       => __( 'Number of Gift Cards', 'woocommerce-gift-cards' ),
				'description' => __( 'Number of gift cards expired.', 'woocommerce-gift-cards' ),
				'type'        => 'integer',
			...
			'expired_balance'  => array(
				'description' => __( 'Expired Balance.', 'woocommerce-gift-cards' ),
				'type'        => 'number',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
				'format'      => 'currency',
		...
		$schema = array(
			...
			'title'      => 'report_giftcards_expired_stats',
			'type'       => 'object',
			'properties' => array(
				'totals'    => array(
	...
	public function set_default_report_data( $results ) {
		if ( empty( $results ) ) {
			...
			$results->totals->giftcards_count    = 0;
			$results->totals->expired_balance    = 0;
	...
	public function get_collection_params() {
		...
		$params['interval']   = array(
			'description'       => __( 'Time interval to use for buckets in the returned data.', 'woocommerce-gift-cards' ),
			'type'              => 'string',
			'default'           => 'week',
			'enum'              => array(
				'hour',
		...
		$params['segmentby']  = array(
			'description'       => __( 'Segment the response by additional constraint.', 'woocommerce-gift-cards' ),
			'type'              => 'string',
			'enum'              => array(
				'product',
				'category',
		...
		$params['fields']     = array(
			'description'       => __( 'Limit stats fields to the specified items.', 'woocommerce-gift-cards' ),
			'type'              => 'array',
