...
/**
 * WC_PRL_Analytics_Data_Store class.
 *
 * @version 2.0.0
 */
abstract class WC_PRL_Analytics_Data_Store extends ReportsDataStore implements DataStoreInterface {
	...
	protected function update_interval_boundary_dates( $start_datetime, $end_datetime, $time_interval, &$intervals ) {
		...
		foreach ( $intervals as $key => $interval ) {
			...
			$datetime->setTimestamp( $interval['datetime_anchor'] );
	...
	public function parse_date_for_sql( $date ) {
		...
		} elseif( wc_prl_is_unix_timestamp( $date ) ) {
			$value = absint( $date );
		} else {
			$value = $date;
		}
