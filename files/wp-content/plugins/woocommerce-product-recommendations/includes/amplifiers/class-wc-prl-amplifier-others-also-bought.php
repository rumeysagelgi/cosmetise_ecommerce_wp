...
class WC_PRL_Amplifier_Others_Also_Bought extends WC_PRL_Amplifier {
	...
	public function run_step( $step_index, $deployment, $args = array() ) {
		...
		$time_period_in_weeks = (int) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_time_weeks', 'oab', $deployment );
		...
		switch ( $step_index ) {
			...
			case 1:
				...
				$sql                           = "SELECT count( distinct( `customer_id` ) )
						...
						;
				$total_customers_bought_source = (int) $wpdb->get_var( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				...
				$customers_count_min = (int) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_foreground_customer_count_min', 'oab', $deployment );
				if ( empty( $total_customers_bought_source ) || $total_customers_bought_source < $customers_count_min ) {
					if ( wc_prl_debug_enabled() ) {
						WC_PRL()->log( sprintf( '[Failed] Step "%d" for deployment #%d and product #%d failed with status `%s`.', $step_index, $deployment->get_id(), $source_id, "Total customers purchased the source product: {$total_customers_bought_source}. Reference Count Threshold: {$customers_count_min}" ), 'info', 'wc_prl_generator_tasks' );
					}
					return null;
				}
			...
			case 2:
				...
				$having_support_percentage      = (float) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_foreground_support_pct_min', 'oab', $deployment );
				$support_number                 = max( (int) $total_customers_bought_source * $having_support_percentage / 100, (int) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_foreground_count_min', 'oab', $deployment ) );
				$results                        = array();
				...
				if ( $total_customers_bought_source ) {
					 ...
					 */
					$foreground_query_type = (string) apply_filters( 'woocommerce_prl_amp_oab_background_query_type', 'subquery', $deployment );
				...
				if ( empty( $counts ) ) {
					if ( wc_prl_debug_enabled() ) {
						$support_needed = ( $having_support_percentage / 100 ) * $total_customers_bought_source;

						$percentage             = sprintf( '%1$s%% of %2$s = %3$s', $having_support_percentage,
							$total_customers_bought_source,
							$support_needed );
						...
						$status = sprintf(
							...
							Required at least %1$s customers.
							%2$s',
							$support_number,
			...
			case 3:
				...
				$sql             = "SELECT count( distinct( `customer_id` ) )
						...
						;
				$total_customers = (int) $wpdb->get_var( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				...
				if ( ! $total_customers ) {
					if ( wc_prl_debug_enabled() ) {
						$status = 'Not enough customers in store. That doesn\'t look normal.';
						WC_PRL()->log( sprintf( '[Failed] Step "%1$d" for deployment #%2$d and product #%3$d failed with status `%4$s`.', $step_index, $deployment->get_id(), $source_id, $status ), 'info', 'wc_prl_generator_tasks' );
					}
					return null;
				}
			...
			case 4:
				...
				$background_support_min_count = (int) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_background_count_min', 'oab', $deployment );
			...
			case 5:
				...
				if ( ! empty( $products_to_calc ) && 0 != $total_customers_bought_source ) {
					 ...
					 */
					$significance_score = WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_oab_significance_score', 'oab', $deployment );

					if ( 'jlh' === $significance_score ) {
						$jlh_threshold = (float) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_jlh_threshold', 'oab', $deployment );
					} elseif ( 'gnd' === $significance_score ) {
						$gnd_threshold = (float) WC_PRL()->amplifiers->get_calibration_option( 'woocommerce_prl_amp_oab_gnd_threshold', 'oab', $deployment );
					}
