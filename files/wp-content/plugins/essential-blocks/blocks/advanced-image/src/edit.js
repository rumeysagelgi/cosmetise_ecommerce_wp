...
export default function Edit(props) {
    ...
    useEffect(() => {
        if (image.sizes && imageSize && imageSize.length > 0) {
            ...
            if (oldImageData?.media_details?.sizes) {
                ...
                const newWidth = oldImageData.media_details.sizes?.[imageSize]
                    ?.width
                    ? oldImageData.media_details.sizes?.[imageSize]?.width
                    : oldImageData.width;
                const newHeight = oldImageData.media_details.sizes?.[imageSize]
                    ?.height
